...
train_split = 3
test_split = 3
seq = generate_sequence(train_split)
nb_iteration= len(sequence) // batch_size 
#load test data
X_test, y_test = load_test_data(test_split)
...
for ep in range(nb_epoch):
    ...
    for it in range(nb_iteration):
        split_seq = seq[it*batch_size:(it+1)*batch_size]
        X_train, y_train= load_train_data(split_seq)
	...
	if it%10==0 and it>0 or it==nb_iteration-1:
            ...
            #test_select = np.sort(test_sequence[0:200])
